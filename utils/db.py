...
class mysql:
    ...
    @staticmethod
    def search(model: str, **kvargs) -> dict:

        total_rows = mysql.fetchone("SELECT COUNT(*) total FROM {}".format(model))
        if total_rows:
            total_rows = total_rows.get("total", 0)
        ...
        return {"result": data, "total_rows": total_rows}
